...
var constraints: [(CGFloat, CGFloat)] = [(0.001, 2.75), (2.75, 4), (4, UITouch().maximumPossibleForce)]
// MARK: - Game State
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first {
            if view.traitCollection.forceTouchCapability == .available {
                if touch.force > constraints[1].0 {
 
                    NotificationCenter.default.addObserver(forName: joystickNotificationName, object: nil, queue: OperationQueue.main) { (notification) in
                    guard let userInfo = notification.userInfo else { return }
                    let data = userInfo["data"] as! AnalogJoystickData
                    self.hero.position = SCNVector3(self.hero.position.x, self.hero.position.z, self.hero.position.y + Float(data.velocity.y * joystickHeightMultiplier))

        }
    }
                if touch.force > constraints[2].0{
                    NotificationCenter.default.addObserver(forName: joystickNotificationName, object: nil, queue: OperationQueue.main) { (notification) in
                        guard let userInfo = notification.userInfo else { return }
                        let data = userInfo["data"] as! AnalogJoystickData
                        self.hero.position = SCNVector3(self.hero.position.x, self.hero.position.z, self.hero.position.y)
